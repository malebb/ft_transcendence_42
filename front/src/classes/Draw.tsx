...
export default class Draw {
  skins: Skin[] = [];
  mapList = [
    { name: "basic", path: "./images/basic.png" },
  ...
  initFont() {
    ...
    return new FontFace(
      "Poppins",
      "url('http://localhost:3000/fonts/Poppins-SemiBold.ttf')"
    );
  }
  ...
  initGameMap(mapName: string): HTMLImageElement {
    ...
    this.mapList.forEach((map) => {
      if (map.name === mapName) img.src = map.path;
    });
    return img;
  }
  ...
  getTextZone(text: string, posX: number, posY: number) {
    ...
    return {
      posX: posX - textzone.actualBoundingBoxLeft,
      posY: posY - textzone.fontBoundingBoxAscent,
  ...
  skin(name: string): LinkZone {
    ...
    let skin: Skin = new Skin(
      ...
      this.ctx!.canvas.height / nbInRow
    );
    let skinZone: LinkZone = {
      posX:
        (this.ctx!.canvas.width / nbInRow) * (this.skins.length % nbInRow) +
        this.ctx!.canvas.width / (nbInRow * 2),
      posY: skin.height * 2 * Math.floor(this.skins.length / nbInRow) + margin,
      width: skin.width,
      height: skin.height,
    };
  ...
  map(): LinkZone[] {
    ...
    this.mapList.forEach((map) => {
      ...
      let mapData: MapData = {
        ...
        mapZone: {
          posX:
            (this.ctx!.canvas.width / nbInRow) * (maps.length % nbInRow) +
            this.ctx!.canvas.width / (nbInRow * 2) -
            width / 2,
  ...
  usernames(leftUsername: string, rightUsername: string) {
    this.text(
      leftUsername,
      this.ctx!.canvas.width / 4.5,
      this.ctx!.canvas.height / 1.02,
      15,
      "white",
      CANVAS_FONT
    );
  ...
  speedPowerUp(
    ...
    leftSpeedPowerUp: boolean,
    rightSpeedPowerUp: boolean
  ) {
    if (leftSpeedPowerUp)
      this.ctx!.drawImage(
        logo,
        this.ctx!.canvas.width / 3.03,
        this.ctx!.canvas.height / 1.2,
        40,
        40
      );
  ...
  checkbox(checkbox: HTMLImageElement): LinkZone {
    let checkboxZone: LinkZone = {
      posX: this.ctx!.canvas.width / 1.8,
      posY: this.ctx!.canvas.height / 1.77,
      width: 20,
      height: 20,
    };
  ...
  updateCheckboxStatus() {
    if (this.checkboxStatus) this.checkboxStatus = false;
    else this.checkboxStatus = true;
  }
