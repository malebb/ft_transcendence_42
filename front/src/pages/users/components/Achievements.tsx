...
const Achievements = () => {
  ...
  function printStatus(achievement: AchievementsData) {
    const achievementDone = (
      <img
        width="58px"
        src="http://localhost:3000/images/achievementDone.png"
        alt="achievementDone"
      />
    );
    const achievementToDo = (
      <img
        width="50px"
        src="http://localhost:3000/images/achievementToDo.png"
        alt="achievementToDo"
      />
    );
  ...
  function oneStepAchievements() {
    ...
    if (stats) {
      return oneStepAchievements.map((achievement) => {
        return (
  ...
  function printProgress(achievement: AchievementsData) {
    if (winAchievements.current.includes(achievement.title))
      return (
        <p>
          {stats!.victory <= achievement.goal!
            ? stats!.victory
            : achievement.goal!}{" "}
          / {achievement.goal}
        </p>
    ...
    if (levelAchievements.current.includes(achievement.title))
      return (
        <p>
          {stats!.level <= achievement.goal! ? stats!.level : achievement.goal!}{" "}
          / {achievement.goal}
        </p>
      );
  ...
  function severalStepsAchievements() {
    ...
    if (stats) {
      return severalStepsAchievements.map((achievement) => {
        return (
          <div className="achievement" key={achievement.title}>
            <h3>{achievement.title}</h3>
            <p>{achievement.desc}</p>
