...
export default function Canvas() {
  ...
  useEffect(() => {
    function powerUpEnabled() {
      return (
        (position.current === "left" && leftPlayer.current!.speedPowerUp) ||
    ...
    function powerUp(e: KeyboardEvent) {
      ...
      if (e.key === " " && powerUpEnabled()) {
        socket.current!.emit("speedPowerUp", {
    ...
    const pong = async () => {
      function mouseOnZone(e: MouseEvent, textZone: LinkZone): boolean {
        ...
        if (
          e.clientX - clickZone.left >= textZone.posX &&
          e.clientX - clickZone.left <= textZone.posX + textZone.width
        ) {
          if (
            e.clientY - clickZone.top >= textZone.posY &&
      ...
      function addLink(
        ...
        var drawMousePointer = (e: MouseEvent) => {
          if (mouseOnZone(e, textZone)) canvas!.style.cursor = "pointer";
          else {
            var mouseOnOtherZone: boolean = false;

            zones.forEach((zone) => {
              if (mouseOnZone(e, zone)) mouseOnOtherZone = true;
            });
            if (!mouseOnOtherZone) canvas!.style.cursor = "default";
          }
        };
      ...
      function opponentDisconnection() {
        ...
        let background: HTMLImageElement =
          draw.current!.initOutGameBackground();

        background.onload = function () {
          draw.current!.outGameBackground(background);
          draw.current!.opponentDisconnection();

          let menuZone = draw.current!.text(
            "menu",
            size.current.width / 4,
            size.current.height / 1.3,
            20,
            FONT_COLOR,
            CANVAS_FONT
          );
          let newGameZone = draw.current!.text(
            "new game",
            size.current.width / 1.3,
            size.current.height / 1.3,
            20,
            FONT_COLOR,
            CANVAS_FONT
      ...
      function result(status: string) {
        ...
        background.onload = function () {
          draw.current!.outGameBackground(background);
          if (status === "won") draw.current!.youWon();
          else draw.current!.youLost();
      ...
      function updateSpeedPowerUp(status: boolean, position: string) {
        if (position === "left") leftPlayer.current!.speedPowerUp = status;
        else if (position === "right")
          rightPlayer.current!.speedPowerUp = status;
      }

      async function launchGame() {
        leftPlayer.current = Object.assign(
          new Player(0, 0, 0, 0, 0, "", "", "", null, null),
          room.current!.leftPlayer
        );
        rightPlayer.current = Object.assign(
          new Player(0, 0, 0, 0, 0, "", "", "", null, null),
          room.current!.rightPlayer
        );

        leftPlayer.current.setCtx(ctx.current!);
        rightPlayer.current.setCtx(ctx.current!);

        ball.current = Object.assign(
          new Ball(0, 0, 0, "white", 0, null, null),
          room.current!.ball
        );
        ...
        socket.current!.on("endGame", (winner: string) => {
          stopGame();
          if (winner === position.current) result("won");
          else result("lost");
        });
        ...
        if (room.current!.powerUpMode) {
          socket.current!.on(
            "updateSpeedPowerUp",
            (status: boolean, position: string) => {
        ...
        map.current = draw.current!.initGameMap(
          (await axiosInstance.current!.get("/users/me", {})).data.map
        );
      ...
      async function matchmaking() {
        ...
        background.onload = async function () {
          ...
          let cancelZone = draw.current!.text(
            "cancel",
          ...
          let playerData: PlayerData = {
            ...
            username: "",
            skin: "",
            powerUpMode: powerUpMode.current,
          };
        ...
        background.onload = function () {
          ...
          skinLinkZones.forEach((skinLinkZone) => {
            addLink(
              ...
              changeSkin,
              skinLinkZones,
              colouredSkins[skinLinkZones.indexOf(skinLinkZone)]
            );
          });
        };
      }
        ...
        background.onload = function () {
          ...
          mapLinkZones.forEach((mapLinkZone: LinkZone) => {
            addLink(
              ...
              changeMap,
              mapLinkZones,
              draw.current!.mapList[mapLinkZones.indexOf(mapLinkZone)].name
            );
          });
        };
      }
        ...
        background.onload = function () {
          ...
          checkbox.onload = function () {
            ...
            checked.onload = function () {
              ...
              let skinsZone = draw.current!.text(
                "skins",
                size.current.width / 4,
                size.current.height / 1.3,
                20,
              ...
              let mapsZone = draw.current!.text(
                "maps",
                size.current.width / 1.3,
                size.current.height / 1.3,
                20,
              ...
              draw.current!.text(
                "Power-up",
                size.current.width / 2.1,
                size.current.height / 1.66,
                16,
                FONT_COLOR,
                CANVAS_FONT
              );
              ...
              addLink(checkboxZone, switchToPowerUpMode, zones, {
                checkbox: checkbox,
                checked: checked,
                zones: zones,
              });
      ...
      function game() {
        ...
        draw.current!.score(
          leftPlayer.current!.score,
          rightPlayer.current!.score
        );
        draw.current!.usernames(
          trimUsername(leftPlayer.current!.username),
          trimUsername(rightPlayer.current!.username)
        );
        draw.current!.speedPowerUp(
          speedPowerUp.current!,
          leftPlayer.current!.speedPowerUp,
          rightPlayer.current!.speedPowerUp
        );
        ball.current?.draw();
      }
    ...
    pong();
    return () => {
      if (socket.current != null) stopGame();
    };
  }, []);
  return (
    <center>
