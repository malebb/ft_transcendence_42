...
const History = () => {
  ...
  const printHistory = () => {
    if (sorted) {
      ...
      return historyElem.current.map((elem) => {
        if (elem.type === "game") {
          return (
            <div className="historyElem" id="gamePlayed" key={elem.value.date}>
              <div className="gameDataContainer">
                <h3 className="leftPlayer">
                  {trimUsername(elem.value.leftUsername)}
                </h3>
                <h3 className="score">
                  {elem.value.leftScore} - {elem.value.rightScore}
                </h3>
                <h3 className="rightPlayer">
                  {trimUsername(elem.value.rightUsername)}
                </h3>
              </div>
            </div>
          );
          ...
          return (
            <div
              ...
              key={elem.value.date}
            >
              <h3 className="title">{elem.value.title}</h3>
              <h4 className="desc">{elem.value.desc}</h4>
            </div>
          );
  ...
  useEffect(() => {
    const initHistory = async () => {
      ...
      const username = (await axiosInstance.current.get("users/me", {})).data
        .email;
      axiosInstance.current = await axiosToken();
      const gamePlayed = (
        await axiosInstance.current.get("history/gamePlayed/" + username)
      ).data.gamePlayed;
      axiosInstance.current = await axiosToken();
      const achievementDone = (
        await axiosInstance.current.get("history/achievementsDone/" + username)
      ).data.achievementDone;
      ...
      for (let i = 0; i < achievementDone.length; ++i) {
        historyElem.current.push({
          type: "achievement",
          value: achievementDone[i],
        });
      }
